...
// Clases para links en mobile bottom sheet — más grandes para touch
function mobileNavLinkClasses(isActive: boolean): string {
  return [
    'flex items-center justify-between px-4 py-3.5 rounded-xl text-base font-medium',
    'transition-all duration-200 ease-out',
    'outline-none focus-visible:ring-2 focus-visible:ring-[#ff6200]',
    isActive
      ? 'text-[#ff6200] bg-[#fff3e8]'
      : 'text-[#3d1400] hover:text-[#ff6200] hover:bg-[#fff3e8]',
  ].join(' ');
}
    ...
    <>
      ...
      >
        ...
        <div className='flex justify-center pt-3 pb-1'>
          <div className='w-10 h-1 rounded-full bg-[#d0c0b0]' />
        </div>
        ...
        <nav className='px-4 pt-2 pb-4'>
          <ul className='flex flex-col gap-1'>
            {NAV_LINKS.map((link) => (
              <li key={link.href}>
                <a
                  href={link.href}
                  className={mobileNavLinkClasses(activePath === link.href)}
                  onClick={() => setMenuOpen(false)}
                  aria-current={activePath === link.href ? 'page' : undefined}
                >
                  {link.label}
          ...
          {/* CTAs mobile */}
          <div className='flex flex-col gap-2 mt-4 pt-4 border-t border-[#f0e8e0]'>
            <Button variant='secondary' size='md' href='/area-clientes'>
              <svg
                width='14'
                height='14'
                viewBox='0 0 24 24'
